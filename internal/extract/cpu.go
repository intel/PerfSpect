...
// NumaCPUListFromOutput returns the NUMA node CPU list from lscpu output.
...
// PPINsFromOutput returns the unique PPINs from MSR output.
...
// ChannelsFromOutput returns the number of memory channels from CPU characteristics.
...
// TurboEnabledFromOutput returns the turbo/boost status.
...
// ChaCountFromOutput returns the CHA count from MSR output.
...
// NumaBalancingFromOutput returns the NUMA balancing status.
...
// ClusteringModeFromOutput returns the clustering mode based on microarchitecture and NUMA configuration.
...
// HyperthreadingFromOutput returns the hyperthreading status from script outputs.
